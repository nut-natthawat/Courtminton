...
const CourtCard = ({ id, name, time, isAvailable, onClick }: CourtCardProps) => {
  return (
    <div 
      className={cn(
        "bg-white p-5 rounded-lg shadow-sm flex flex-col cursor-pointer transition-all duration-300 hover:shadow-md border border-gray-100 hover:scale-105",
        isAvailable ? "hover:border-court-orange/100" : "opacity-70"
      )}
      onClick={isAvailable ? onClick : undefined}
        >
      <div className="badminton-court mb-4 overflow-hidden rounded-md">
        <div className="badminton-court-lines border-2"></div>
      </div>
